...
public class PointUsingSuccessPopupController {
    ...
    private void handleNumberClick(ActionEvent event) {
        ...
        confirmButton.setOnAction(e -> {
            try {
                int usedPoints = Integer.parseInt(phoneField1.getText());
                System.out.println("Confirm button clicked. Used points: " + usedPoints);
                // buyController의 총 가격 업데이트
                if (buyController != null) {
                    buyController.updateTotal(usedPoints);
                }
                // 팝업 닫기
                Stage stage = (Stage) confirmButton.getScene().getWindow();
                stage.close();
            } catch (NumberFormatException ex) {
                // 예외 처리 (예: 입력값이 비어 있을 경우)
            }
        });
    ...
    public void init() {
        currentPoint = memberDto.getAvailablePoint(); // memberDto에서 현재 포인트 가져오기
